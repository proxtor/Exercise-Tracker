...
app.get("/api/users/:_id/logs", async (req, res) => {
  ...
  const id = req.params._id;  // Extract user ID from the URL path

  // Find the user by ID
  const user = await User.findById(id);
  if (!user) {  // Corrected: Check if the user was found
    res.send("Could not find user");
    return;
  }

  // Initialize a filter object to apply date range filters if they exist
  let dateObj = {};
  if (from) {
  ...
  // Construct the filter for exercises
  let filter = {
    user_id: id,
  };
  if (from || to) {
  ...
  // Query the Exercise collection with the filter and limit
